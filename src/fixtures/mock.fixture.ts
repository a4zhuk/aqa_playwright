...
import {
  ICustomerResponse,
  ICustomersResponse,
} from "types/salesPortal/customer.types";
import { IMetricsResponse } from "types/salesPortal/metrics.type";
...
class Mock {
  ...
  async metric(body: IMetricsResponse, statusCode: STATUS_CODES = STATUS_CODES.OK) {
    this.page.route(apiConfig.BASE_URL + "/" + apiConfig.ENDPOINTS.METRIKS, 
        async (route) => {
            await route.fulfill({
                status: statusCode,
                contentType: "application/json",
