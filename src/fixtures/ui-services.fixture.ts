...
import { test as base } from "fixtures/api-service.fixture";
...
import { ProductsUISetvice } from "ui/services/products/products.ui-service";
import { AddNewProductUISetvice } from "ui/services/products/add-new-product.ui-service";
...
interface IUIServices {
  ...
  productsUIService: ProductsUISetvice;
  addNewProductUIService: AddNewProductUISetvice;
}
...
export const test = base.extend<IUIServices>({
  ...
  productsUIService: async ({ page }, use) => {
    await use(new ProductsUISetvice(page));
  },
  addNewProductUIService: async ({ page }, use) => {
    await use(new AddNewProductUISetvice(page));
  },
});
